...
class pts // class for manipulaitng and displaying pointclouds or polyloops in 3D 
 ...
 void savePts(String fn) 
  ...
  int n(int v,int t){return (v+t)%nv;}
  int p(int v) {if(v==0) return nv-1; else return v-1;}
  int p(int v,int t) {if(v<t) return v-t+nv; else return v-t;}
  
  pt tuck(pt A, pt B,pt C,float s)
  {
    return P(B,s,V(V(B,A),V(B,C))); //<>//
  }
  pts subdivideJarekInto(pts Q,float s)
  {
      float tucks = 2.0*s-2.0; //<>//
      pts t1 = new pts();
      pts t2 = new pts();
      pts t3 = new pts();
      t1.declare();
      ...
      t2.empty();
       t3.declare();
      t3.empty();
      for(int i=0; i<Q.nv; i+=1)
      {
        t1.addPt(Q.G[i]);
        t1.addPt(P(Q.G[i],Q.G[Q.n(i)]));
      }
      for (int i = 0;i<t1.nv;i++)
      {
        t2.addPt(tuck(P(t1.G[i],t1.G[t1.p(i)]),t1.G[i],P(t1.G[i],t1.G[t1.n(i)]),1));
      }
     for (int i = 0;i<t2.nv;i++)
      {
        t3.addPt(tuck(P(t2.G[i],t2.G[t2.p(i)]),t2.G[i],P(t2.G[i],t2.G[t2.n(i)]),tucks));
      }
    return t3;
  }
  pts subdivideFourPointInto(pts Q)
  {
    return subdivideJarekInto(Q,0);
  }
  pts subdivideCubicInto(pts Q)
  {
    return  subdivideJarekInto(Q,1); //<>//
  }
  pts subdivideQuinticInto(pts Q)
  {
    return  subdivideJarekInto(Q,1.5);
  }
 ...
 // pts subdivideDemoInto(pts Q) 
  ...
  void displaySkater() 
      ...
      if(showSkater) 
        ...
        step = nv/100+1;
        int width = 15*step*step;
       vec forward = new vec();
       forward.setTo(U(V(G[n(f)].x-G[p(f)].x,G[n(f)].y-G[p(f)].y,G[n(f)].z-G[p(f)].z)));

       if (animating)
       {
         vec BC = V(G[n(f,step)],G[n(f,step+1)]);
         vec AC = V(G[n(f,step-1)],G[n(f,step+1)]);
         float ACB = angle(BC,AC);
         float h = norm(BC)*cos(ACB);
         float r = norm(AC)*norm(AC)/(2*h);
         if (start == 0)
         {
           leftfoot.setTo(P(G[f],width,V(V(G[n(f)],G[f]),V(G[p(f)],G[f]))));
           rightfoot.setTo(P(G[f],width,V(V(G[n(f,step-1)],G[n(f,step)]),V(G[n(f,step)],G[p(n(f,step+1))]))));
           start = 1;
         }
         else 
         {
           if (footcount == 1)
           {
             leftfoot.setTo(P(G[f],width,V(V(G[n(f,step+1)],G[n(f,step)]),V(G[n(f,step-1)],G[n(f,step)]))));
             footcount = 0;
           }
           else
           {
             rightfoot.setTo(P(G[f],width,V(V(G[n(f,step)],G[n(f,step+1)]),V(G[n(f,step)],G[n(f,step-1)]))));
             footcount = 1;
           }
         }
        }
        showDancer(leftfoot , 0, rightfoot, forward ); 
      }
      //fill(red); arrow(B[f],P(G[f],50,V(V(G[n(f)],G[f]),V(G[p(f)],G[f]))),20);} //
      }
