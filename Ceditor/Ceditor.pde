...
void draw() {
  ...
  for(int i=0; i<level; i++) 
    {
    Q.copyFrom(R); 
    //if(method==5) {R.copyFrom(Q.subdivideDemoInto(R));}
    if(method==4) {R.copyFrom(Q.subdivideQuinticInto(R));}
    if(method==3) {R.copyFrom(Q.subdivideCubicInto(R)); }
    if(method==2) {R.copyFrom(Q.subdivideJarekInto(R,0.5)); }
    if(method==1) {R.copyFrom(Q.subdivideFourPointInto(R));}
    if(method==0) {R.copyFrom(Q.subdivideQuadraticInto(R)); }
    }
  R.displaySkater();
